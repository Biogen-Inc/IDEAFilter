...
In the application you can freely select a subset of columns to include in the filter. You can now freely set what columns users are able to add to the filter. You should note these columns can still be set using pre-selection and will still be applied to the filter. For instance, you can see below that only `height` has been selected but `gender` is still being applied.
...
Looking at the second example is informative on how a user can create their own pre-selections.
